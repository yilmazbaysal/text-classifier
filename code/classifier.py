class Classifier:
    ...
    def find_sense(self, test_data):
        ...
        """
        maximum = (-1, "")  # (Probability, Sense ID)
        for sense_id in self.train_data.keys():
            ...
            probability = len(self.train_data[sense_id]) / self.instance_count  # class_count / total_class_count
            for token in test_data:
                if token in self.vocabulary:
                    ...
                    probability *= ((token_count + 1) / (instance_word_counts + self.V))
